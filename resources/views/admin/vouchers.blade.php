<x-app-layout title="Manajemen Voucher">
    ...
    <div class="py-12" x-data="{ 
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-700/50">
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Voucher</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Diskon</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Kuota</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Berlaku</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                <td class="px-6 py-4 whitespace-nowrap font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">{{ $voucher->code }}</td>
        ...
        <!-- Voucher Modal (Combined Add/Edit) -->
        <template x-if="addOpen || editOpen">
            <div class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
                <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    <div class="fixed inset-0 bg-gray-900 bg-opacity-70 transition-opacity" @click="addOpen = false; editOpen = false"></div>
                    <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                    
                    <div class="inline-block align-bottom bg-gray-50 dark:bg-gray-900 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        <form :action="editOpen ? '/admin/vouchers/' + form.id : '{{ route('admin.vouchers.store') }}'" method="POST">
                            ...
                            {{-- Modal Header --}}
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 flex justify-between items-center">
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white" x-text="editOpen ? 'Edit Voucher' : 'Tambah Voucher'"></h3>
                                <button type="button" @click="addOpen = false; editOpen = false" class="text-gray-400 hover:text-gray-600">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-4">
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest">Detail Voucher</h4>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-2 flex bg-gray-100 dark:bg-gray-900 p-1 rounded-xl">
                                            <button type="button" @click="form.discount_type = 'percent'" :class="form.discount_type === 'percent' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500'" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all duration-200">
                                                Persentase (%)
                                            </button>
                                            <button type="button" @click="form.discount_type = 'fixed'" :class="form.discount_type === 'fixed' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500'" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all duration-200">
                                                Nominal (Rp)
                                            </button>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="discount_amount" x-model="form.discount_amount" required class="block w-full pl-10 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="10">
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" name="min_order" x-model="form.min_order" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="0">
                                        </div>
                                        <div>
                                            <x-input-label value="Maks. Pakai" />
                                            <input type="number" name="max_uses" x-model="form.max_uses" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="100">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-4">
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest">Periode Berlaku</h4>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="date" name="valid_from" x-model="form.valid_from" class="block w-full pl-10 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="date" name="valid_until" x-model="form.valid_until" class="block w-full pl-10 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 text-center">
                                <button type="submit" class="w-full py-3 bg-gray-900 dark:bg-indigo-600 hover:bg-gray-800 dark:hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition shadow-lg">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    Simpan Voucher
                                </button>
                                <button type="button" @click="addOpen = false; editOpen = false" class="mt-2 text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                    Batal dan kembali
                                </button>
                            </div>
                        </form>
